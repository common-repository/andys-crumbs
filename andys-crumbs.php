...
/*
Copyright 2009 Andrew Bellamy  (admin@andrew-bellamy.co.uk)
...
function crumbs()
{
	...
	if(is_archive()) {
		$datetime = get_gmt_from_date($post->post_date);
		$temp = explode(" ", $datetime);
		$datetime = explode("-", $temp[0]);
		
		$y = '<a href="'.get_year_link($datetime[0]).'">'.$datetime[0].'</a>';
		$m = '<a href="'.get_month_link($datetime[0], $datetime[1]).'">'.date('F', mktime(0, 0, 0, $datetime[1], $datetime[2], $datetime[0])).'</a>';
		$d = '<a href="'.get_day_link($datetime[0], $datetime[1], $datetime[2]).'">'.date('d', mktime(0, 0, 0, $datetime[1], $datetime[2], $datetime[0])).'</a>';
		if(is_year()) {
	...
	echo '<div id="andys-crumbs">'.$title.'</div>';
}
?>